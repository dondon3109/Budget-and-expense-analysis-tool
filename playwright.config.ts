...
import { e2eEnvGuardError } from "./e2e/env-guard";
...
// The refusal decision lives in e2e/env-guard.ts so scripts/e2e-env-guard.test.mjs can cover it
// without a subprocess. It runs here at module scope, so it fails `--list` and every project
// selection, not only a full run.
const guardError = e2eEnvGuardError(envFileLoaded, process.env);
