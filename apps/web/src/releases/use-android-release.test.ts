...
const VALID_METADATA = {
  version: "0.2.3-beta",
  versionCode: 20303,
  downloadUrl: "https://downloads.zoption.site/android/zoption-beta-0.2.3.apk",
...
describe("useAndroidRelease", () => {
  it("loads validated remote metadata with a simple CORS GET", async () => {
    ...
    expect(result.current.release?.versionName).toBe("0.2.3-beta");
    await waitFor(() => expect(vi.mocked(fetchMock)).toHaveBeenCalled());
    expect(result.current.release?.versionCode).toBe(20303);
  ...
  it("does not treat an unmount abort as unavailable", async () => {
    ...
    expect(result.current.release?.downloadPath).toContain("zoption-beta-0.2.3.apk");
  });
