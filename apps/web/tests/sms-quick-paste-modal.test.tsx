...
describe("SmsQuickPasteModal component", () => {
  ...
  it("does not render modal when isOpen is false", () => {
    render(<SmsQuickPasteModal isOpen={false} onClose={onClose} onApply={onApply} />);

    expect(screen.queryByRole("dialog")).toBeNull();
  });

  it("renders modal when isOpen is true", () => {
    render(<SmsQuickPasteModal isOpen={true} onClose={onClose} onApply={onApply} />);

    expect(screen.getByRole("dialog")).toBeDefined();
    expect(screen.getByText("Quick Paste from SMS / Alert")).toBeDefined();
    expect(screen.getByPlaceholderText(/e.g. Your card ending in 4321/i)).toBeDefined();
  });

  it("parses text on paste or type into textarea and autofills form inputs", async () => {
    render(<SmsQuickPasteModal isOpen={true} onClose={onClose} onApply={onApply} />);

    const textarea = screen.getByPlaceholderText(/e.g. Your card ending in 4321/i);
    ...
    const merchantInput = screen.getByLabelText(/Merchant \/ Description/i) as HTMLInputElement;
    const accountInput = screen.getByLabelText(/Account \/ Card/i) as HTMLInputElement;
