...
describe("handleQueuedJob", () => {
  it("dispatches PayPal, bug-report, and account-deletion messages", async () => {
    reconcileBillingCheckout.mockResolvedValue({ outcome: "pending" });
    retryNotification.mockResolvedValue(true);
    reconcileUser.mockResolvedValue("deleted");

    await handleQueuedJob(env, { type: "billing-reconcile", tenantId: "user:1" });
    ...
    expect(reconcileBillingCheckout).toHaveBeenCalledWith(billingRepository, env, "user:1");
