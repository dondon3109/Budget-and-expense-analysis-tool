...
describe("AccountDeletionService", () => {
  it("reauthenticates the JWT subject, purges D1, then clears Storage before Auth", async () => {
    ...
    await expect(
      service.deleteAccount({ env: ENV, user: USER, accessToken: "access-token", password: "password" }),
    ).resolves.toBe("deleted");

    expect(steps).toEqual(["current-user", "password", "d1", "storage", "storage-mark", "auth", "auth-mark"]);
  ...
  it("does not purge data when the current password cannot be verified", async () => {
    ...
    await expect(
      service.deleteAccount({ env: ENV, user: USER, accessToken: "access-token", password: "wrong" }),
  ...
  it("keeps the tombstone and schedules a retry if Storage cleanup fails", async () => {
    ...
    const gateway = createGateway({ purgeAvatars: vi.fn(async () => Promise.reject(new Error("down"))) });
    const service = createAccountDeletionService(repository, () => gateway);
