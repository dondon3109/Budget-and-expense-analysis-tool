...
const EXTRACTION_PROMPT = [
  ...
  "  Include every readable product or paid fee line, using the extended price when a line shows quantity times unit price. Exclude VATable sales, VAT-exempt sales, zero-rated sales, and VAT amount summaries. Include each discount, coupon, promotion, senior citizen or PWD discount, and Less VAT deduction as its own item with a negative amountMinor. Exclude receipt headers, subtotal, grand total, payment, tendered cash, and change. Items including discounts should add up to amountMinor. Use [] if no individual line is readable.",
