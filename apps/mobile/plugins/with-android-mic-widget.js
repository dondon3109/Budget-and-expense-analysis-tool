...
function widgetProviderKt() {
  return [
    ...
    ' * and expands with a "Record expense" label when resized wider.',
    " */",
...
function widgetIntentsKt() {
  return [
    ...
    '  private val LEADER = Regex("""^(i(\'ve| have)?|we(\'ve| have)?|please|kindly)?\\s*(just\\s+)?(spent|spend|paid|pay|log|logged|add|added|bought|buy|purchase|purchased|record|recorded|have\\s+spent|had)\\b\\s*""", RegexOption.IGNORE_CASE)',
    ...
    '  private val PAYMENT_METHOD = Regex("""\\b(using|via|with|through|from|on|by|paid\\s+with|paid\\s+via|charged\\s+to)\\s+(my\\s+)?(cash|gcash|maya|paymaya|card|credit\\s+card|debit\\s+card|bank|bank\\s+transfer|wallet|checking|savings)\\b|\\b(in\\s+cash|by\\s+card)\\b""", RegexOption.IGNORE_CASE)',
    '  private val TIME_WORDS = Regex("""\\b(today|yesterday|tonight|this\\s+morning|this\\s+afternoon|this\\s+evening|just\\s+now|earlier(\\s+today)?|last\\s+night)\\b""", RegexOption.IGNORE_CASE)',
    '  private val LEADING_PREP = Regex("""^(for|on|at|in|to|about|around|a|an|the|my|some)\\b\\s*""", RegexOption.IGNORE_CASE)',
    '  private val TRAILING_PREP = Regex("""\\s+\\b(for|on|at|in|to|using|with|via|from|by|and)\\s*$""", RegexOption.IGNORE_CASE)',
    "",
    ...
    "  }",
    "",
    "  fun summarizeMerchant(raw: String): String {",
    "    var text = raw",
    '      .replace(CURRENCY, " ")',
    '      .replace(LEADER, "")',
    ...
    "    while (prev != text) {",
    "      prev = text",
    '      text = text.replace(LEADING_PREP, "")',
    "    }",
    '    text = text.replace(TRAILING_PREP, "")',
    "    text = tidy(text)",
    '    if (text.isEmpty()) return "Expense"',
    ...
    "    }",
    "    val merchant = summarizeMerchant(withoutAmount).take(240)",
