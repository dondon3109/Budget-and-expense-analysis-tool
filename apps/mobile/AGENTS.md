...
- The daily reminder (`src/features/reminders/daily-reminder.ts`) is a local notification scheduled on the device; there is no push service. `POST_NOTIFICATIONS` and `RECEIVE_BOOT_COMPLETED` come from the expo-notifications library manifest. Its config plugin is deliberately not listed in `app.config.ts`, because the plugin's iOS mod adds the `aps-environment` push entitlement. Every identity change (`clearUserScopedRuntimeState`) cancels the reminder, so it never outlives the account that set it. Jest picks up `__mocks__/expo-notifications.js` automatically, because the real module runs native checks on import.
