...
- The daily reminder (`src/features/reminders/daily-reminder.ts`) is a local notification scheduled on the device; there is no push service. `POST_NOTIFICATIONS` and `RECEIVE_BOOT_COMPLETED` come from the expo-notifications library manifest. Its config plugin is deliberately not listed in `app.config.ts`, because the plugin's iOS mod adds the `aps-environment` push entitlement.
