...
- The daily reminder (`src/features/reminders/daily-reminder.ts`) is a local notification scheduled on the device; there is no push service. `POST_NOTIFICATIONS` and `RECEIVE_BOOT_COMPLETED` come from the expo-notifications library manifest. Its config plugin is deliberately not listed in `app.config.ts`, because the plugin's iOS mod adds the `aps-environment` push entitlement. `applyDailyReminder` saves the chosen time only once the OS schedule matches it, and `useDailyReminderSession` (mounted in `app/_layout.tsx`) re-applies the saved time without prompting once the session is signed in and clears it when the session resolves signed out, so the card never shows a time with nothing scheduled and a session that ended while the app was closed leaves no reminder behind. Every identity change (`clearUserScopedRuntimeState`) cancels the reminder, and an apply still in flight across one cannot bring it back, so it never outlives the account that set it. Jest picks up `__mocks__/expo-notifications.js` automatically, because the real module runs native checks on import.
