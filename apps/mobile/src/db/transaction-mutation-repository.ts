...
export class LocalTransactionMutationRepository {
  ...
  private async currentOutbox(entityId: string) {
    const row = await this.database.getFirstAsync(
      `SELECT operation_id, idempotency_key, entity_type, entity_id, operation_type,
       ...
       FROM sync_outbox WHERE entity_type = 'transaction' AND entity_id = ?`,
      entityId,
    );
    if (!row) return null;
  ...
  updateTransaction(id: string, value: TransactionUpdate): Promise<void> {
    ...
    return this.writer.run(async () => {
      await this.database.withTransactionAsync(async () => {
        ...
        const outbox = await this.currentOutbox(id);
    ...
    return this.writer.run(async () => {
      await this.database.withTransactionAsync(async () => {
        for (const result of response.results) {
          ...
          const outbox = await this.currentOutbox(operation.entityId);
          ...
          if (result.status === "acknowledged") {
            await this.database.runAsync(
              "UPDATE transactions SET server_revision = ?, sync_state = 'synced' WHERE id = ?",
              result.revision,
              result.entityId,
            );
            await this.database.runAsync(
          ...
          if (result.status === "conflict") {
            ...
            await this.database.runAsync(
              "UPDATE transactions SET sync_state = 'conflicted' WHERE id = ?",
              result.entityId,
            );
            continue;
          }
          ...
          await this.database.runAsync(
            "UPDATE transactions SET sync_state = 'failed' WHERE id = ?",
            result.entityId,
          );
        }
      });
