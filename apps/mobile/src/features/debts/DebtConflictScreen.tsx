...
import { useDebtConflict, useLocalWorkspace } from "@/db/local-workspace-state";
import type { LocalDebtConflictVersion } from "@/db/transaction-mutation-repository";
...
import { debtTypeLabels } from "./debt-form";
...
function VersionCard({
  ...
  title: string;
  version: LocalDebtConflictVersion | null;
}) {
  ...
  return (
    <Card accessibilityLabel={title}>
      ...
      {!version ? (
        <Text style={[typography.body, { color: theme.colors.textMuted }]}>No debt</Text>
      ) : (
        <>
...
export function DebtConflictScreen() {
  const params = useLocalSearchParams<{ id?: string | string[] }>();
  const id = singleParam(params.id);
  const local = useLocalWorkspace();
  const sync = useSyncState();
  const state = useDebtConflict(id);
  const theme = useZoptionTheme();
  ...
  const resolve = async (): Promise<void> => {
    ...
    setMessage(null);
    try {
      await local.workspace.transactionMutations.resolveDebtConflict(id, choice);
      setChoice(null);
      router.back();
      sync.retry();
    } catch (error) {
      setMessage(
        error instanceof Error
          ? error.message
          : "The preserved debt conflict could not be resolved safely.",
      );
      setChoice(null);
    } finally {
      setResolving(false);
  ...
  return (
    ...
    >
      <Stack.Screen options={{ title: "Review debt conflict" }} />
      {state.error ? (
        ...
        <View style={styles.centered}>
          <ErrorState
            ...
            message="There is no unresolved conflict for this debt."
          />
        </View>
      ) : (
        <ScrollView
      ...
      <ConfirmationDialog
        ...
        message={
          choice === "keep_local"
            ...
            : "Zoption will discard this device's proposed debt and replace it with the preserved server debt."
        }
        onCancel={() => setChoice(null)}
        onConfirm={() => void resolve()}
        title={choice === "keep_local" ? "Keep this device debt?" : "Use server debt?"}
        visible={choice !== null}
      />
    </SafeAreaView>
  );
