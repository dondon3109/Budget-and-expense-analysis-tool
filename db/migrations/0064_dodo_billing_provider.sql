-- Dodo Payments joins PayPal as a checkout provider. The billing tables pin `provider` with a
-- CHECK constraint, which SQLite can only change by rebuilding the table. Every column the
-- deployed Worker reads is kept, so the previous release keeps working against this schema.
-- A Dodo checkout session exists before its subscription does, so checkout references gain
-- `provider_checkout_id`, the provider's session id.
DROP VIEW IF EXISTS `effective_pro_entitlements`;
--> statement-breakpoint
CREATE TABLE `billing_customers_next` (
  ...
  CHECK (`provider` IN ('paypal', 'dodo'))
);
--> statement-breakpoint
CREATE TABLE `billing_checkout_references_next` (
...
--> statement-breakpoint
CREATE TABLE `billing_subscriptions_next` (
  `provider` text NOT NULL,
...
--> statement-breakpoint
CREATE TABLE `billing_webhook_events_next` (
  `provider` text NOT NULL,
...
--> statement-breakpoint
INSERT INTO `billing_customers_next`
...
--> statement-breakpoint
INSERT INTO `billing_checkout_references_next`
  (`id`, `tenant_id`, `provider`, `plan`, `interval`, `provider_plan_id`,
   `provider_subscription_id`, `expires_at`, `completed_at`, `superseded_at`,
   `last_reconciled_at`, `reconciliation_attempts`, `last_provider_status`,
   `last_reconciliation_error`, `created_at`, `updated_at`)
...
--> statement-breakpoint
INSERT INTO `billing_subscriptions_next`
...
--> statement-breakpoint
INSERT INTO `billing_webhook_events_next`
...
--> statement-breakpoint
ALTER TABLE `billing_customers_next` RENAME TO `billing_customers`;
--> statement-breakpoint
ALTER TABLE `billing_checkout_references_next` RENAME TO `billing_checkout_references`;
--> statement-breakpoint
ALTER TABLE `billing_subscriptions_next` RENAME TO `billing_subscriptions`;
--> statement-breakpoint
ALTER TABLE `billing_webhook_events_next` RENAME TO `billing_webhook_events`;
--> statement-breakpoint
...
WHERE `completed_at` IS NULL AND `superseded_at` IS NULL;
--> statement-breakpoint
CREATE UNIQUE INDEX `billing_checkout_references_provider_checkout_unique`
ON `billing_checkout_references` (`provider`, `provider_checkout_id`)
