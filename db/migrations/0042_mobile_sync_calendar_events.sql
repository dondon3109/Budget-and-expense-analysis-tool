ALTER TABLE `calendar_events` ADD `revision` integer NOT NULL DEFAULT 1 CHECK (`revision` > 0);
...
DROP TRIGGER `subscriptions_mobile_sync_insert`;
DROP TRIGGER `subscriptions_mobile_sync_update`;
DROP TRIGGER `subscriptions_mobile_sync_delete`;
--> statement-breakpoint
CREATE TABLE `mobile_sync_changes_new` (
	`tenant_id` text NOT NULL,
	`sequence` integer NOT NULL CHECK (`sequence` > 0),
	`entity_type` text NOT NULL CHECK (`entity_type` IN ('account', 'category', 'transaction', 'budget', 'goal', 'debt', 'subscription', 'event')),
...
--> statement-breakpoint
CREATE VIEW `mobile_sync_event_rows` AS
SELECT
	`tenant_id`,
	`id` AS `entity_id`,
...
SELECT
	`tenant_id`,
	(SELECT `sequence` FROM `mobile_sync_state` WHERE `tenant_id` = events.`tenant_id`)
	...
	`payload_json`,
	`server_updated_at`
FROM `mobile_sync_event_rows` events;
--> statement-breakpoint
...
--> statement-breakpoint
CREATE TRIGGER `calendar_events_mobile_sync_insert`
...
BEGIN
	...
	SELECT `tenant_id`,
		(SELECT `sequence` FROM `mobile_sync_state` WHERE `tenant_id` = NEW.`tenant_id`),
		'event', `entity_id`, `row_revision`, 'upsert', `payload_json`, `server_updated_at`
	FROM `mobile_sync_event_rows`
	WHERE `tenant_id` = NEW.`tenant_id` AND `entity_id` = NEW.`id`;
END;
--> statement-breakpoint
CREATE TRIGGER `calendar_events_mobile_sync_update`
...
END;
--> statement-breakpoint
CREATE TRIGGER `calendar_events_mobile_sync_delete`
...
BEGIN
	...
	INSERT INTO `mobile_sync_changes` VALUES (
		OLD.`tenant_id`,
		(SELECT `sequence` FROM `mobile_sync_state` WHERE `tenant_id` = OLD.`tenant_id`),
		'event', OLD.`id`, OLD.`revision` + 1, 'delete', NULL, datetime('now')
	);
END;
--> statement-breakpoint
