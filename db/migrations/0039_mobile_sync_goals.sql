ALTER TABLE `financial_goals` ADD `revision` integer NOT NULL DEFAULT 1 CHECK (`revision` > 0);
...
CREATE TABLE `mobile_sync_changes_new` (
	...
	`entity_type` text NOT NULL CHECK (`entity_type` IN ('account', 'category', 'transaction', 'budget', 'goal')),
...
END;
--> statement-breakpoint
CREATE VIEW `mobile_sync_goal_rows` AS
SELECT
	`tenant_id`,
	`id` AS `entity_id`,
...
SELECT
	`tenant_id`,
	(SELECT `sequence` FROM `mobile_sync_state` WHERE `tenant_id` = goals.`tenant_id`)
	...
	`payload_json`,
	`server_updated_at`
FROM `mobile_sync_goal_rows` goals;
--> statement-breakpoint
