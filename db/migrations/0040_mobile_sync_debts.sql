ALTER TABLE `debts` ADD `revision` integer NOT NULL DEFAULT 1 CHECK (`revision` > 0);
...
CREATE TABLE `mobile_sync_changes_new` (
	...
	`entity_type` text NOT NULL CHECK (`entity_type` IN ('account', 'category', 'transaction', 'budget', 'goal', 'debt')),
...
END;
--> statement-breakpoint
CREATE VIEW `mobile_sync_debt_rows` AS
SELECT
	`tenant_id`,
	`id` AS `entity_id`,
	...
	json_object(
		...
		'aprBasisPoints', `apr_basis_points`,
...
SELECT
	`tenant_id`,
	(SELECT `sequence` FROM `mobile_sync_state` WHERE `tenant_id` = debts.`tenant_id`)
	...
	`payload_json`,
	`server_updated_at`
FROM `mobile_sync_debt_rows` debts;
--> statement-breakpoint
