...
- Supabase email/password and Google signup/login flows, verified-email identity linking, confirmation, recovery, session refresh, and sign-out.
...
- Configure Google provider credentials in Preview and Production Supabase projects, including the Google callback URL.
- Prove the same-verified-email Google flow adds an identity to one existing Supabase user ID and opens the same D1 tenant without duplicating a workspace.
