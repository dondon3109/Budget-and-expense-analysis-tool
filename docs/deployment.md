...
1. The `review` job runs with `contents: read`. Claude Code (`claude -p`, model `claude-opus-5-5`, on `CLAUDE_CODE_OAUTH_TOKEN`) reads the diff with read-only tools and returns a schema-checked verdict: `approve` or `changes_requested`, findings, and whether the Conventional Commit type fits. It uploads that verdict as the `claude-review` artifact.
