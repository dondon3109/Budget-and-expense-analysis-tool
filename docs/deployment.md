...
2. The `draft` job runs with `permissions: contents: read`. It fetches that report by id, runs Claude Code headless (`claude -p`, model `claude-opus-5`) with file tools and `pnpm vitest run` only, checks its own output with `scripts/bugfix-scrub.mjs`, and uploads `fix.patch`, `pr-body.md`, and `meta.json` as the `bugfix-draft` artifact.
...
| `ANTHROPIC_API_KEY`  | Repository secret, read by Claude Code in the draft job. Give it a spend limit: a test the agent writes runs with it   | `gh secret list`                                                 |
