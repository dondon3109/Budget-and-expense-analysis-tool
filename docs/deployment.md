...
3. With `AUTO_MERGE_PRS` set to `true`, an eligible pull request gets an approval from `github-actions[bot]` and squash auto-merge, pinned to the reviewed head commit. GitHub merges it once the CI gate ruleset's checks pass. It is eligible only when all of these hold: the verdict is `approve` with no blocker or major finding, the title is a Conventional Commit whose type fits, no path matches `scripts/pr-risk.mjs`, it changes at most 5,000 lines, and a human opened it. Anything else keeps its comment and waits for a human merge. A push dismisses the approval, so a new commit must be reviewed again.
