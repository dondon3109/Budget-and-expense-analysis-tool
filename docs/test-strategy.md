...
- **Domain tests:** money parsing, totals, transfers, budget percentages, account snapshot and credit-debt calculations, strict assistant inputs, duplicate fingerprints, header-aware CSV parsing, import date normalization, Amount/Debit/Credit mapping, and validation edge cases.
...
- **API tests:** retired public-data endpoint, unauthenticated/invalid-token rejection, HTTPS-only non-local identity configuration, CORS preflight, security headers, authenticated tenant propagation, write and bulk-read rate-limit identity, media-type/body-size enforcement, strict request validation, transaction/category/budget/import/export contracts, account balance ownership, assistant tool allowlisting and server-owned tenant context, Debit/Credit and slash-date normalization, commit-time category override authorization, atomic rejection, and stable failures.
- **Frontend tests:** route guards, intended-destination redirects, landing account actions, static-preview labeling, bearer-token attachment, one refresh retry, final unauthorized sign-out, user-scoped query keys, assistant consent and plain-text rendering, account balance entry, first-use dashboard detection, bank preset matching, header selection, PHP confirmation, import pagination, cross-page bulk categories, workbook-client lifetime, input/drop parity, and persistent theme behavior.
