...
The public `0.2.3-beta` APK does not contain `expo-updates` and cannot receive
OTA updates. Do not publish an OTA until a later OTA-capable APK is live and
verified.
...
3. Choose a new native app version and Android `versionCode` through the normal
   signed-app release process. Do not reuse `0.2.3-beta` / `20303`.
