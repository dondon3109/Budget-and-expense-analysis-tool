...
| Unlocked phone opens the finance app             | Optional per-subject 6-digit PIN; lock screen on cold start and after 60 seconds in the background; salted hash in SecureStore, never the PIN; attempt cooldown; forgetting it means signing out, which respects the unsynced-changes guard   |
