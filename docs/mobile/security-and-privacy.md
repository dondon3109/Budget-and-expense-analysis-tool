...
| Unlocked phone opens the finance app             | Optional per-subject app password; lock screen on cold start and after 60 seconds in the background; salted hash in SecureStore, never the password; attempt cooldown; forgetting it means signing out, which respects the unsynced-changes guard |
