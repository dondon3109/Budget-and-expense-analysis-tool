...
describe("a11y source audit", () => {
  ...
  it("flags an icon-only button and passes one with an aria-label", () => {
    expect(scanFixture('export const A = () => <button type="button"><X aria-hidden="true" /></button>;')).toEqual([
      "no-accessible-name",
    ]);
    expect(
      scanFixture('export const A = () => <button type="button" aria-label="Close"><X aria-hidden="true" /></button>;'),
    ).toEqual([]);
  });
  ...
  it("counts a name nested inside a span", () => {
    ...
    expect(
      scanFixture('export const A = () => <button type="button"><Mic aria-hidden="true" /><span>Voice Entry</span></button>;'),
    ).toEqual([]);
  });

  it("stays quiet when a name comes from a variable it cannot resolve", () => {
    expect(scanFixture("export const A = ({ label }) => <button type=\"button\">{label}</button>;")).toEqual([]);
  });
  ...
  it("flags an unlabelled control and passes a labelled or wrapped one", () => {
    ...
    expect(scanFixture('export const A = () => <input aria-label="Search" value="" onChange={() => {}} />;')).toEqual([]);
    expect(
      scanFixture('export const A = () => <label>Name<input value="" onChange={() => {}} /></label>;'),
    ).toEqual([]);
  ...
  it("flags focusable content inside an aria-hidden subtree", () => {
    expect(
      scanFixture('export const A = () => <div aria-hidden="true"><button type="button">Hi</button></div>;'),
    ).toEqual(["focusable-inside-aria-hidden"]);
    expect(scanFixture('export const A = () => <div aria-hidden="true"><span>Decorative</span></div>;')).toEqual([]);
  });

  it("flags a positive tabindex", () => {
    expect(scanFixture('export const A = () => <div tabIndex={2}>x</div>;')).toEqual(["positive-tabindex"]);
    expect(scanFixture('export const A = () => <div tabIndex={-1}>x</div>;')).toEqual([]);
  });

  it("flags an image with no alt", () => {
    expect(scanFixture('export const A = () => <img src="/a.png" />;')).toEqual(["img-without-alt"]);
